...
try {
    switch ($action) {
        ...
        case 'add_comentario':
            ...
            if (!$texto) jsonError('Comentário vazio.');
            ...
            $newId = (int)$db->lastInsertId();
            jsonSuccess('Comentário adicionado.', ['id' => $newId, 'nome' => $user['nome'], 'comentario' => sanitize($texto), 'created_at' => date('Y-m-d H:i:s')]);
            break;
        ...
        case 'delete_comentario':
            ...
            if ($comId <= 0) jsonError('ID inválido.');
            // Só o autor ou admin pode apagar
            $stmt = $db->prepare('SELECT utilizador_id FROM especificacao_comentarios WHERE id = ?');
            $stmt->execute([$comId]);
            $com = $stmt->fetch();
            if (!$com) jsonError('Comentário não encontrado.');
            if ($com['utilizador_id'] != $user['id'] && !in_array($user['role'], ['super_admin', 'org_admin'])) jsonError('Sem permissão.');
            $db->prepare('DELETE FROM especificacao_comentarios WHERE id = ?')->execute([$comId]);
            jsonSuccess('Comentário eliminado.');
            break;
